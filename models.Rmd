...
#model2 <- lm(log(discounted_value) ~  treatment * Number.of.queries + Год_постройки + Этажность  + Количество_квартир + Высота_потолков + Спортивная_площадка + Детская_площадка + Газоснабжение + Пассажирских_лифтов + Грузовых_лифтов + PCA_Component + РУДН + Тютчевская ,
